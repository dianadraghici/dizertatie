...
export class ProjectComponent implements OnInit {
    ...
    private isPercentAndStatusConditionValidated(index: number): Boolean {
        ...
        return ( (percentIdFC.valid && statusIDFC.valid) &&
                    ((percentIdFC.value == 0 && statusIDFC.value == 0) ||   // for Certitude = 0% -> Status = 'ABANDON'
                    (percentIdFC.value == 10 && (statusIDFC.value == 3 || statusIDFC.value == 2)) ||    // for Certitude = 100% -> Status = 'TERMINE' || 'VALIDE'
                    (percentIdFC.value >= 1 && percentIdFC.value <= 9) && (statusIDFC.value == 1)) );     // for  10% <= Certitude <= 90 -> Status = 'ETUDE'
    }
